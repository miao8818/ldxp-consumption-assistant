...
- 双击悬浮球可填写购买页的联系方式和安全密码，但不会自动提交订单
...
- 订单统计匹配 `https://pay.ldxp.cn/order*` 和 `https://www.ldxp.cn/order*`，购买页快捷填写匹配商品页与下单页（`/item/*`、`https://pay.ldxp.cn/shop/*`）
...
首次在商品购买页双击悬浮球，会打开“购买表单设置”。输入自己的联系方式和安全密码后保存；之后在购买页双击悬浮球，扩展会尝试填写页面中标注为“联系方式”和“安全密码”的输入框，不会点击下单、支付或提交按钮。

联系方式和安全密码只保存在当前浏览器的 `chrome.storage.local`，不会写入 GitHub、不会出现在 CSV，也不会发送给扩展作者。更换设备或清除扩展数据后需要重新设置。
...
- 不读取订单卡密、验证码或浏览器 Cookie；购买页安全密码只会在用户主动设置快捷填写后保存在本机。
